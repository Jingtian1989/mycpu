`ifndef __MY_CPU_CPU_H__
`define __MY_CPU_CPU_H__
...
`define BUS_STATE_BUS		1:0		//总线状态总线
`define BUS_STATE_IDLE		2'b00 	//空闲
`define BUS_STATE_REQ		2'b01 	//请求
`define BUS_STATE_ACCESS	2'b10 	//访问
`define BUS_STATE_STALL		2'b11 	//停滞

`define MEM_OP_W 	2
`define MEM_OP_BUS	1:0 		//内存操作总线
`define MEM_OP_NOP	2'h0 		//无操作
`define MEM_OP_LDW	2'h1 		//字写入
`define MEM_OP_STW	2'h2     	//字读入
...
`define CREG_ADDR_EXE_MODE_LOCALE	0
`define CREG_INT_ENABLE_LOCALE		1
`define CREG_EXP_CODE_LOCALE		2:0
`define CREG_DLY_FLAG_LOCALE		3

`define RESET_VECTOR				30'h0
`define SH_AMOUNT_BUS				4:0
`define SH_AMOUNT_LOCALE			4:0
